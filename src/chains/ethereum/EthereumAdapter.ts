...
export class EthereumAdapter implements IChainAdapter {
  ...
  async getTransaction(hash: string): Promise<RawTransaction> {
    // Check cache first
    const cached = await cache.get<RawTransaction>(CACHE_KEYS.rawTx('ethereum', hash));
    if (cached) return cached;

    const [tx, receipt] = await Promise.all([
      rpcManager.call('ethereum', (client) =>
    ...
    // Get block timestamp (cached separately since it's immutable)
    ...
    await cache.set(CACHE_KEYS.rawTx('ethereum', hash), raw, TTL.RAW_TX);
    return raw;
  }

  async getTokenMetadata(address: string): Promise<TokenMetadata> {
    const normalized = address.toLowerCase();
    const cached = await cache.get<TokenMetadata>(CACHE_KEYS.tokenMetadata('ethereum', normalized));
    if (cached) return cached;

    if (normalized === NATIVE_TOKEN_ADDRESS) {
      return { address: normalized, symbol: 'ETH', decimals: 18, name: 'Ether' };
    }

    const [symbol, decimals, name] = await Promise.all([
      rpcManager.call('ethereum', (c) =>
    ...
    await cache.set(CACHE_KEYS.tokenMetadata('ethereum', normalized), metadata, TTL.TOKEN_METADATA);
    return metadata;
  }

  async isContractAddress(address: string): Promise<boolean> {
    const code = await rpcManager.call('ethereum', (c) =>
  ...
  private async getBlockTimestamp(blockNumber: number): Promise<number> {
    const cacheKey = CACHE_KEYS.blockTimestamp('ethereum', blockNumber);
    const cached = await cache.get<number>(cacheKey);
    if (cached !== null) return cached;

    const block = await rpcManager.call('ethereum', (c) =>
