...
```
API 服务:    api.yourdomain.com  →  服务器公网 IP:6000
前端:        yourdomain.com      →  服务器公网 IP:3001
```
...
# 设置后端 API 地址（生产地址）
export API_PROXY_TARGET=http://127.0.0.1:6000
...
module.exports = {
  apps: [
    ...
    },
    {
      ...
      env_production: {
        NODE_ENV: 'production',
        API_PROXY_TARGET: 'http://127.0.0.1:6000',
      },
...
cd /opt/tokensee/web
API_PROXY_TARGET=http://127.0.0.1:6000 node_modules/.bin/next start --port 3001 &

# 或者用 PM2 单独启动
pm2 start --name tokensee-web node_modules/.bin/next start --port 3001 --cwd /opt/tokensee/web
```
...
```nginx
# 前端：yourdomain.com → Next.js :3001
server {
    listen 80;
...
```nginx
# API: api.yourdomain.com → 后端 :6000
server {
    listen 80;
